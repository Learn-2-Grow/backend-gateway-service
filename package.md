# Installed Packages


Generated on: 2025-11-03T19:55:11.971Z

Node: v20.17.0  •  npm: 10.8.2

<!--PACKAGE-TABLE-START-->
| Package | Installed | Declared |
|---|---:|---|
| `@nestjs/cli` | `10.4.9` | `^10.0.0` |
| `@nestjs/common` | `10.4.20` | `^10.0.0` |
| `@nestjs/config` | `4.0.2` | `^4.0.2` |
| `@nestjs/core` | `10.4.20` | `^10.0.0` |
| `@nestjs/platform-express` | `10.4.20` | `^10.0.0` |
| `@nestjs/schematics` | `10.2.3` | `^10.0.0` |
| `@nestjs/testing` | `10.4.20` | `^10.0.0` |
| `@prisma/client` | `6.19.0` | `^6.19.0` |
| `@types/express` | `4.17.23` | `^4.17.17` |
| `@types/jest` | `29.5.14` | `^29.5.2` |
| `@types/node` | `20.19.20` | `^20.3.1` |
| `@types/pg` | `8.15.6` | `^8.15.6` |
| `@types/supertest` | `6.0.3` | `^6.0.0` |
| `@typescript-eslint/eslint-plugin` | `8.46.0` | `^8.0.0` |
| `@typescript-eslint/parser` | `8.46.0` | `^8.0.0` |
| `class-transformer` | `0.5.1` | `^0.5.1` |
| `class-validator` | `0.14.2` | `^0.14.2` |
| `eslint` | `8.57.1` | `^8.42.0` |
| `eslint-config-prettier` | `9.1.2` | `^9.0.0` |
| `eslint-plugin-prettier` | `5.5.4` | `^5.0.0` |
| `husky` | `9.1.7` | `^9.1.7` |
| `jest` | `29.7.0` | `^29.5.0` |
| `joi` | `18.0.1` | `^18.0.1` |
| `pg` | `8.16.3` | `^8.16.3` |
| `prettier` | `3.6.2` | `^3.0.0` |
| `prisma` | `6.19.0` | `^6.19.0` |
| `reflect-metadata` | `0.2.2` | `^0.2.0` |
| `rxjs` | `7.8.2` | `^7.8.1` |
| `source-map-support` | `0.5.21` | `^0.5.21` |
| `supertest` | `7.1.4` | `^7.0.0` |
| `ts-jest` | `29.4.5` | `^29.1.0` |
| `ts-loader` | `9.5.4` | `^9.4.3` |
| `ts-node` | `10.9.2` | `^10.9.1` |
| `tsconfig-paths` | `4.2.0` | `^4.2.0` |
| `typescript` | `5.9.3` | `^5.1.3` |
<!--PACKAGE-TABLE-END-->
**Summary**: Declared: 27 · Installed: 27
